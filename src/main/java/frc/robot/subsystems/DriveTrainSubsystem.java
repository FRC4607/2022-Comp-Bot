...
private ADIS16470_IMU gyro;
    ...
    */
    ...
    - It's based on radians, so all angles are internally stored as a x and y coordinate on the unit circle, or cos and sin components respectively.
    - Therefore, Rotation2d is CCW positive, just like the radian system.
    - This also means you must be really careful to not pass in any degree measures without converting them first.
    - Rotation2d goes from -inf to inf, so you can use it continuously.
    */

    /**
     * Returns the robot's heading, ranging from -inf to inf.
     * @param ccwPositive Whether the reading should be returned treating counterclockwise as positive or negative, with true meaning ccw is positive.
     * @return The rotation of the robot in degrees, with counterclockwise rotations increasing this value.
     */
    ...
    public double getGyroscopeReading(boolean ccwPositive) {
        ...
        double corrected = continuousHeading % 180;

        // Even half rotations = regular, odd = trouble. 
        if (Math.floor(continuousHeading / 180) % 2 == 0) {
            return corrected;
        }
        else {
            /*
            This took a while for me to figure out, so here is how this whole thing works:
            Even half rotation? No problem, just return it whether it is positve or negative.
            Odd hald rotation? It needs to be corrected. The correction for positve numbers is x - 180,
            while the calculation for negative numbers is x + 180. To fix this, multiply 180 by the sign
            of the continuous rotation value. 180 * 1 = 180, making x - 180. 180 * -1 = -180, making x + 180.
            */
            return corrected - (180  * Math.signum(continuousHeading));
        }
    }

    /**
     * Gets the x acceleration of the robot in m/s^2 (i think). You can check which axis is which direction and sign by looking at the graphic on the roborio.
    ...
    /**
     * Gets the y acceleration of the robot in m/s^2 (i think). You can check which axis is which direction and sign by looking at the graphic on the roborio.
    ...
    /**
     * Gets the z acceleration of the robot in m/s^2 (i think). You can check which axis is which direction and sign by looking at the graphic on the roborio.
